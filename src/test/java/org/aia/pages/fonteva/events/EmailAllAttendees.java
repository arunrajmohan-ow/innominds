...
public class EmailAllAttendees {
	...
	public void clickEmailAdress()throws InterruptedException {
	     Thread.sleep(5000);
		String expectedStatus = "Active"; // Assuming this is the status to compare with
		util.waitUntilElement(driver, eventButton);
		// Assuming eventButton is a WebElement and s is the text of the element, and comparing the text content
		...
		if (actualStatus.equalsIgnoreCase(expectedStatus)) {
			...
			Assert.assertTrue(emailText.isDisplayed(), "You are about to send mass emails to 3 TestQAMedium11072023130156 recipients");		System.out.println("You are about to send mass emails to 3 TestQAMedium11072023130156 recipients text is displayed");
			...
			util.waitUntilElement(driver, yesButton);
			...
			if (status.equalsIgnoreCase(expectedStatus)) {
			    ...
			    Assert.assertTrue(emailTextProceed.isDisplayed(), "Do you want to proceed?'");
				util.waitUntilElement(driver,yesButton );		
				Utility.highLightElement(driver, yesButton);
				...
				if (yesButton.isDisplayed()) {
					System.out.println("yes is displayed");
					 util.clickUsingJS(driver, yesButton);
						Thread.sleep(5000);
						System.out.println("VERIFIED: click yes button");
						util.waitUntilElement(driver,toastmessage );
						Utility.highLightElement(driver, toastmessage);
						util.acceptAlert();
						//Assert.assertTrue(wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//b[text()='Manage Adapters']"))).isDisplayed());
						Assert.assertTrue(toastmessage.isDisplayed(), "Mass Email Sent Successful! The session agenda has been emailed to each attendee registered to the event");
						System.out.println("Mass Email Sent Successful! The session agenda has been emailed to each attendee registered to the event");
				} else {
					Assert.fail();
				}
	...
	public void clickEmailAdressNo() throws InterruptedException {
		...
		if (actualStatus.equalsIgnoreCase(expectedStatus)) {
			...
			Assert.assertTrue(emailText.isDisplayed(),
					"You are about to send mass emails to 3  recipients");
			System.out.println(
					"You are about to send mass emails to 3  recipients text is displayed");
			util.waitUntilElement(driver, emailTextProceed);
			Utility.highLightElement(driver, emailTextProceed);
			Assert.assertTrue(emailTextProceed.isDisplayed(), "Do you want to proceed?'");
