...
//		Assert.assertTrue(addressInputField.isDisplayed(), "addressInputField field is not available");
		...
		Assert.assertTrue(closeButonOfCreateVenuePopUp.isDisplayed(), " close Buton Of Create Venue PopUp is not available");
	...
	public void createNewVenue(String description,Map<String,String>address,String imageURL) throws AWTException, InterruptedException {
	...
	public void newlyCreatedVenueIsDisplayedInVenuesListingScreen() throws InterruptedException {
		...
		Assert.assertTrue(value, "Assert failure:- "+EventConfig.venueNameInputField+" is not dispayed in the venue listing screen");
	}
	...
	public void VerifyTotalCreatedVenueCounts() {
		...
		if(Integer.parseInt(EventConfig.getVenueRecordsCount2.replaceAll("[a-zA-z() ]", ""))>Integer.parseInt(EventConfig.getVenueRecordsCount.replaceAll("[a-zA-z() ]", ""))) {
			value =true;
		}
	}
	public void editCreatedVenueNameAndVerifyIt() throws InterruptedException {
		...
		util.scrollingElementUsingJS(driver, driver.findElement(By.xpath(VenuesEvent.venueEditIcon(EventConfig.venueNameInputField))));
...
public void deleteVenueNameAndVerifyIt() throws InterruptedException {
		...
		util.scrollingElementUsingJS(driver, driver.findElement(By.xpath(VenuesEvent.venueDeleteIcon(EventConfig.venueNameInputField))));
		...
		Assert.assertFalse(value, "Assert failure:- "+EventConfig.venueNameInputField+" is not deleted yet, it is dispayed in the venue listing screen");
	}
